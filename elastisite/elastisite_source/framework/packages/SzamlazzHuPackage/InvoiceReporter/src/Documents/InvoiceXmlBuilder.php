...
use Trianity\SzamlazzHu\Documents\Templates\InvoiceXml\PdfSettingsTemplate;

class InvoiceXmlBuilder implements DocumentBuilder
{
    ...
    {
        $data = XmlHeaders::ACTIONS['GET_COMMON_INVOICE']['schema'];
    ...
    public function saveXml(): DocumentBuilder
    {
        ...
        if (! Storage::disk('local')->put(
            ...
            Log::error("[trianity/szamlazzhu] Couldn't store Query Invoice XML temporary XML");
            $this->pdf->xmlPath = '';
    ...
    public function createRequest(): DocumentBuilder
    {
        if (Storage::disk('local')->exists('szamla/'.$this->pdf->xmlFileName)) {
            ...
            $param = [
                'action-szamla_agent_xml' => $xml_file,
                'generate' => 'Query invoice xml',
            ];
        ...
        Log::error("[trianity/szamlazzhu] Couldn't create Query Invoice XML SzamlaAgent request, xml: {$this->pdf->xmlPath}.");
        $this->pdf->apiAnswer = '';

        return $this;
    }

    public function parseRequest(): InvoiceXmlData
    {
        return ParseInvoiceXmlQuery::handle($this);
    }
