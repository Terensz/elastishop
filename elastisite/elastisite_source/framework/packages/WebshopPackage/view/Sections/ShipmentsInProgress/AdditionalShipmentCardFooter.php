...
?>
<?php if ($shipmentDataSetRow['shipment']['permittedForCurrentUser'] && in_array($shipmentDataSetRow['shipment']['status'], Shipment::STATUS_COLLECTION_SHIPMENTS_HANDLING_PROMPTED_TO_USER)): ?>
...
<?php else: ?>
    <?php if ($shipmentDataSetRow['shipment']['permittedUserType'] == ShipmentService::PERMITTED_USER_TYPE_GUEST): ?>
    <div class="card-footer">
        ...
        2.: <a class="" href="" onclick="Webshop.bindShipmentToAccount(event, '<?php echo $shipmentDataSetRow['shipment']['code']; ?>')">A megrendeléshez hozzákapcsoljuk ezt a felhasználói fiókot</a>, amivel épp be van jelentkezve, ám ebben az esetben kijelentkezve soha többé nem lesz elérhető a megrendelése.<br>
    </div>
    <?php else: ?>
        <?php  
        /**
