...
class UserAccountRegistrationTokenRepository extends DbRepository
{
    public function findByUserAccountId($userAccountId)
    {
        $stm = "SELECT token
                FROM user_account_registration_token
    ...
    {
        $stm = "UPDATE user_account_registration_token
                SET redeemed_at = now()
    ...
    {
        $stm = "INSERT INTO user_account_registration_token (user_account_id, token) VALUES (:userAccountId, :token)";
    ...
    {
        $stm = "SELECT id FROM user_account_registration_token WHERE token = :token ";
