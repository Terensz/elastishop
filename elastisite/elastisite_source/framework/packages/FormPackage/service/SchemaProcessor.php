...
// dump($childRepo->getPrimaryKeyField()); //exit;
                ...
                if ($primaryEntityPos !== false) {
                    ...
                    // dump($primaryEntityPos);exit;

                    $this->form->setPrimaryRepository($childRepo);
                    ...
                    $entityCollector = $em->createEntityCollection(
                        ...
                        'schemaProcessor', // uniqueIdForTesting
                        false
                    );
    ...
    public function processAllPosted()
    {
        ...
        foreach ($this->form->getSpecsMap() as $propertyAlias => $propertyParams) {
            ...
            if ($this->form->isSubmitted() && !$propertyParams['technical'] && $processedRequestKey) {
                ...
                /**
                 * @todo This solution is bad, we have to put this into a loop, this will bind same value to every unprocessed
                */
...
// dump('alma');
        ...
        $primaryRepository = $this->form->getPrimaryRepository();
        if ($primaryRepository && $primaryRepository->getRepositoryType() == $primaryRepository::REPOSITORY_TYPE_TECHNICAL) {
            return true;
        }
    ...
    public function validateBuiltInRule($ruleName, $posted, $ruleValue, $form, $propertyAlias)
    {
        ...
        if ($ruleName == 'required' && $ruleValue == true) {
            ...
            //     dump($form);exit;
            // }
            if ($posted === '0' || ($posted && ctype_digit($posted)) || ($posted && $posted !== '') || $posted === false) {
                return [
                    'result' => true,
                    'message' => null
                ];
