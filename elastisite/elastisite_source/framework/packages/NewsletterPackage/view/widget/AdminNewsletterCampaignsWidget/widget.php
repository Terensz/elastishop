<div id="adminNewsletterCampaigns-list-container"></div>

<script>
    $('document').ready(function() {
        console.log('docready');
        NewsletterCampaign.list();
    });
    var NewsletterCampaign = {
        list: function() {
            $.ajax({
                'type': 'POST',
                'url': '<?php echo $container->getUrl()->getHttpDomain(); ?>/admin/newsletter/campaigns/list',
                'data': {
                },
                'async': true,
                'success': function(response) {
                    // console.log(response);
                    $('#adminNewsletterCampaigns-list-container').html(response.view);
                },
