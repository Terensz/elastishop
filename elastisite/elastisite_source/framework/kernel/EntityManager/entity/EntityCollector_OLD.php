...
class EntityCollector extends Kernel
{
    ...
    public function isMultiple($parentId, $entityName)
    {
        // dump($parentId);
        // dump($entityName);
        if ($parentId == 0) {
        ...
        if (!isset($parentEntity->getEntityAttributes()['relations'][$entityName])) {
            dump($parentEntity->getEntityAttributes());
        }

        return $parentEntity->getEntityAttributes()['relations'][$entityName]['multiple'];
	}
    ...
    )
    {
        ...
        $collectionElement = array(
            ...
            'entityKey' => $entityKey,
            'multiple' => $this->isMultiple($parentId, $entityNameParts[1]),
            'entity' => $entity
        );
        if ($test) {
