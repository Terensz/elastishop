...
class AscScaleRepository extends DbRepository 
{
    ...
    */
    public function getScaleAccessibility(int $ascScaleId, int $projectUserId = null) : array
    {
        ...
        */
        ...
        $personalProjectTeamsOfScale = $dbm->findAll($stm, $params);
        
        $sharedNonParentUnitIds = [];
        $sharedParentUnitIds = [];
        foreach ($personalProjectTeamsOfScale as $personalProjectTeam) {
            /**
             * If we have at least one row, than the scale is partially accessible for sure.
            */
            $partiallyAccessible = true;
            if ($personalProjectTeam['asc_unit_id']) {
                if ($personalProjectTeam['children_included'] == 1) {
                    $sharedParentUnitIds[] = $personalProjectTeam['asc_unit_id'];
                } else {
                    $sharedNonParentUnitIds[] = $personalProjectTeam['asc_unit_id'];
                }
            }
            if (count($personalProjectTeam) == 1 && !$personalProjectTeam['asc_unit_id']) {
                /**
        ...
        */
        ...
        if ($partiallyAccessible && !$fullyAccessible) {
            ...
            // dump($unitPermissionOverrides);
            // dump($accessibleUnitIds);
        }
    ...
    */
    ...
    public function getUnitPermissionOverrides(array $ascUnitIds, int $projectUserId = null) : array
    {
        ...
        INNER JOIN project_team_user ptu ON ptu.project_team_id = pt.id 
        INNER JOIN project_user pu ON pu.id = ptu.project_user_id 
        INNER JOIN project_team_user_unit_permission_override ptuupo ON ptuupo.project_team_user_id = ptu.id AND ptuupo.asc_unit_id IN (".$ascUnitIdPlaceholders.")
        WHERE pu.id = :project_user_id 
