...
class AscPermissionService extends Service
{
    ...
    public static function getAccessibility(int $ascScaleId, int $projectUserId, int $ascUnitId = null) : array
    {
        App::getContainer()->wireService('projects/ASC/repository/AscScaleRepository');
        $scaleRepo = new AscScaleRepository();
        $scaleAccessibility = $scaleRepo->getScaleAndUnitAccessibility($ascScaleId, $projectUserId, $ascUnitId);
        ...
        $unitSpecificPermissionOverrides = [];
        if ($ascUnitId) {
            if (isset($scaleAccessibility['permissionOverrides'][$ascUnitId])) {
                $unitSpecificPermissionOverrides = $scaleAccessibility['permissionOverrides'][$ascUnitId];
            }
        }

        $permissionOverrides = $ascUnitId ? $unitSpecificPermissionOverrides : $scaleAccessibility['permissionOverrides'];
    ...
    {
        if (isset(self::$cache['scalePermissions'])) {
            return self::$cache['scalePermissions'];
        }
        ...
        */
        ...
        $scalePermissions = self::assemblePermissionsArray([
            'forceAllFalse' => false,
            'isOwned' => $isOwned,
            'onProject' => $onProject,
        ...
        self::$cache['scalePermissions'] = $scalePermissions;
    ...
    public static function assemblePermissionsArray($options = [])
    {
        ...
        return [
            self::PERMISSION_SCALE_OWNERSHIP => $forceAllFalse ? false : ($isOwned),
            self::PERMISSION_VIEW_SCALE => $forceAllFalse ? false : ($isOwned),
