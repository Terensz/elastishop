...
var nicEditorConfig = bkClass.extend({
    buttons: {
        'bold': {
            name: Translator.trans('Click to Bold'),
            command: 'Bold',
            tags: ['B', 'STRONG'],
            css: {
                'font-weight': 'bold'
        ...
        'italic': {
            name: Translator.trans('Click to Italic'),
            command: 'Italic',
            tags: ['EM', 'I'],
            css: {
                'font-style': 'italic'
        ...
        'underline': {
            name: Translator.trans('Click to Underline'),
            command: 'Underline',
            tags: ['U'],
            css: {
        ...
        'left': {
            name: Translator.trans('Left Align'),
            command: 'justifyleft',
            noActive: true
        },
        'center': {
            name: Translator.trans('Center Align'),
            command: 'justifycenter',
            noActive: true
        },
        'right': {
            name: Translator.trans('Right Align'),
            command: 'justifyright',
            noActive: true
        },
        'justify': {
            name: Translator.trans('Justify Align'),
            command: 'justifyfull',
            noActive: true
        },
        'ol': {
            name: Translator.trans('Insert Ordered List'),
            command: 'insertorderedlist',
            tags: ['OL']
        },
        'ul': {
            name: Translator.trans('Insert Unordered List'),
            command: 'insertunorderedlist',
            tags: ['UL']
        },
        'subscript': {
            name: Translator.trans('Click to Subscript'),
            command: 'subscript',
            tags: ['SUB']
        },
        'superscript': {
            name: Translator.trans('Click to Superscript'),
            command: 'superscript',
            tags: ['SUP']
        },
        'strikethrough': {
            name: Translator.trans('Click to Strike Through'),
        ...
        // 'explain': {
        //     name: Translator.trans('Click to mark Word explain'),
        //     command: 'explain',
        //     tags: ['EXP']
        // },
        'removeformat': {
            name: Translator.trans('Remove Formatting'),
            command: 'removeformat',
            noActive: true
        },
        'indent': {
            name: Translator.trans('Indent Text'),
            command: 'indent',
            noActive: true
        },
        'outdent': {
            name: Translator.trans('Remove Indent'),
            command: 'outdent',
            noActive: true
        },
        'hr': {
            name: Translator.trans('Horizontal Rule'),
            command: 'insertHorizontalRule',
            noActive: true
        }
    },
    iconsPath: htmlEditorConfig.iconsPath,
...
var nicEditor = bkClass.extend({
    ...
    checkReplace: function(B) {
        ...
        if (typeof(A) != 'undefined' && A !== null && typeof(A.removeInstance) == 'function') {
            // console.log(A);
            A.removeInstance(B);
            A.removePanel();
        }
    ...
    addInstance: function(B, C) {
        B = this.checkReplace($BK(B));
        if (typeof(B) != 'undefined' && B !== null && (B.contentEditable || !!window.opera)) {
...
var nicEditorInstance = bkClass.extend({
    ...
    blur: function() {
        ...
        if (typeof(this.elm) != 'undefined' && this.elm !== null) {
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
    ...
    initFrame: function() {
        if (typeof(this.elmFrame) != 'undefined' && this.elmFrame !== null) {
...
var nicSelectOptions = {
    buttons: {
        'fontSize': {
            name: Translator.trans('Select Font Size'),
            type: 'nicEditorFontSizeSelect',
            command: 'fontsize'
        },
        'fontFamily': {
            name: Translator.trans('Select Font Family'),
            type: 'nicEditorFontFamilySelect',
            command: 'fontname'
        },
        'fontFormat': {
            name: Translator.trans('Select Font Format'),
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({
    ...
    init: function() {
        this.setDisplay(Translator.trans('Font&nbsp;Size...'));
        for (itm in this.sel) {
...
var nicEditorFontFamilySelect = nicEditorSelect.extend({
    ...
    init: function() {
        this.setDisplay(Translator.trans('Font&nbsp;Family...'));
        for (itm in this.sel) {
...
var nicEditorFontFormatSelect = nicEditorSelect.extend({
    ...
    init: function() {
        this.setDisplay(Translator.trans('Font&nbsp;Format...'));
        for (itm in this.sel) {
...
var nicLinkOptions = {
    buttons: {
        'link': {
            name: Translator.trans('Add Link'),
            type: 'nicLinkButton',
            tags: ['A']
        },
        'unlink': {
            name: Translator.trans('Remove Link'),
            command: 'unlink',
            noActive: true
        }
    }
...
var nicLinkButton = nicEditorAdvancedButton.extend({
    addPane: function() {
        ...
        this.addForm({
            "": {
                type: "title",
                txt: Translator.trans('Add/Edit Link')
            },
            href: {
                type: "text",
                txt: "URL",
            ...
            title: {
                type: "text",
                txt: Translator.trans('Title')
            },
            target: {
                type: "select",
                txt: Translator.trans('Open In'),
                options: {
                    "": Translator.trans('Current Tab'),
                    _blank: Translator.trans('New Tab')
                },
                style: {
                    width: "100px"
                }
    ...
    submit: function(C) {
        ...
        if (A == "http://" || A == "") {
            alert(Translator.trans('You must enter a URL to Create a Link'));
            return false
        }
        this.removePane();
        if (!this.ln) {
...
var nicColorOptions = {
    buttons: {
        'forecolor': {
            name: Translator.trans('Change Text Color'),
            type: 'nicEditorColorButton',
            noClose: true
        },
        'bgcolor': {
            name: Translator.trans('Change Background Color'),
...
var nicImageOptions = {
    buttons: {
        'image': {
            name: Translator.trans('Add Image'),
            type: 'nicImageButton',
            tags: ['IMG']
        }
    }
...
var nicImageButton = nicEditorAdvancedButton.extend({
    addPane: function() {
        if (typeof(this.ne.selectedInstance.selElm()) == 'undefined') {
            console.log('this.ne.selectedInstance is undefined');
            return false;
        }
        console.log(this.ne);
    ...
    submit: function(B) {
        ...
        if (this.im) {
            let imageAlign = this.inputs.align.value;
            let imageAlignUcFirst = imageAlign.charAt(0).toUpperCase() + imageAlign.slice(1);
            this.im.setAttributes({
                src: this.inputs.src.value,
                alt: this.inputs.alt.value,
                align: this.inputs.align.value,
                class: 'contentImage imageAlign' + imageAlignUcFirst
            });
...
var nicSaveOptions = {
    buttons: {
        'save': {
            name: Translator.trans('Save this content'),
...
var nicCodeOptions = {
    buttons: {
        'xhtml': {
            name: Translator.trans('Edit HTML'),
            type: 'nicCodeButton'
        }
    }
...
var nicCodeButton = nicEditorAdvancedButton.extend({
    ...
    addPane: function() {
        this.addForm({
            "": {
                type: "title",
                txt: Translator.trans('Edit HTML')
            },
            code: {
                type: "content",
...
var nicUploadOptions = {
    buttons: {
        'upload': {
            name: Translator.trans('Upload Image'),
...
var nicUploadButton = nicEditorAdvancedButton.extend({
    nicURI: htmlEditorConfig.imageUploadUrl,
    errorText: "ErrorText",
    addPane: function() {
        if (typeof window.FormData === "undefined") {
            return this.onError(Translator.trans('Image uploads are not supported in this browser, use Chrome, Firefox, or Safari instead.'))
        }
        this.im = this.ne.selectedInstance.selElm().parentTag("IMG");
        var A = new bkElement("div").setStyle({
            padding: "10px"
        ...
        new bkElement("div").setStyle({
            ...
            fontWeight: "bold",
            paddingBottom: "5px"
        }).setContent(Translator.trans('Upload Image')).appendTo(A);
    ...
    onError: function(A) {
        this.removePane();
        alert(A || Translator.trans('Failed to upload image'))
    },
    uploadFile: function() {
        var B = this.fileInput.files[0];
        if (!B || !B.type.match(/image.*/)) {
            this.onError(Translator.trans('Only image files can be uploaded'));
            return
        }
    ...
    onUploaded: function(B) {
        var imageLink = B.link;
        var linkParts = imageLink.split("/");
        var imageCode = linkParts[linkParts.length - 1];
        var width = B.width;
        console.log(B);
        this.removePane();
        var D = 'image/' + imageCode;
        // var D = B.link;
        if (!this.im) {
            this.ne.selectedInstance.restoreRng();
        ...
        if (this.im) {
            ...
            this.im.classList.add("contentImage");
        }
    }
});
